...
function CardGrid({ items, navigate }) {
  return (
    <nav className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4 w-full">
...
export default function AppGate({ navigate, items = NAV_ITEMS, title = null }) {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-center p-6 bg-gradient-to-br from-fit-bg to-fit-bg2 text-fit-ink">
      ...
      <div className="w-full max-w-6xl space-y-8 animate-in fade-in zoom-in-95 duration-700">
        {isGrouped
          ? items.filter((g) => g.items?.length > 0).map((g) => (
              <div key={g.group}>
                ...
                <CardGrid items={g.items} navigate={navigate} />
              </div>
            ))
          : <CardGrid items={items} navigate={navigate} />}
      </div>
