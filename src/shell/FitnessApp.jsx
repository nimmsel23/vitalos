...
export default function FitnessApp({ user, recentDays, coverageThreshold, gender, muscleLanguage, taxonomy, dashboardHighlighter, navMode = 'tabs', subTab, onSubTab, sessionDate, sessionDraft, onOpenSession }) {
  const [inspectorExercise, setInspectorExercise] = useState(null)
  const [reviewSubTab, setReviewSubTab] = useState(null)

  const isHomeMode = navMode === 'home'
  const tab = subTab || (isHomeMode ? 'gate' : 'dash')
